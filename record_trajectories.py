...
def train(variant):
    ...
    with open(f"{variant['data_out_path']}/{variant['offline_rl_type']}.json","w") as offline_rl_data:
        ...
        #   print(obs_string)
            ...
            d = {"episode_no" : episode_no, "steps" : [], "question" : questions[0],"answer" : answers[0], "mask" : list(game_finishing_mask[:,0]) }
            steps = {}
            cum_reward = 0
            for step,state in enumerate(states):
                ...
                if len(split_commands) == 3:
                    ...
                    act,obj,mod = *split_commands, "</s>"
                elif len(split_commands) == 1:
                     act = split_commands[0]

                # Add in </s> when mod is empty 

                reward =  round(command_rewards_np[0][step],5)
                cum_reward += reward
                d["steps"].append({"step": step,"state" : "<s> "+ state + " <|> " +questions[0] + " </s>", "command" :  {"action" : act, "modifier" : mod, "object" : obj}, 
                "reward" : reward})

            d["total_reward"] = cum_reward

            print(json.dumps(d),file=offline_rl_data)
